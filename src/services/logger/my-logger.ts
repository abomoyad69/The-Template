...
import { APIColorScheme, DebugColorScheme, ErrorColorScheme, LogColorScheme, VerboseColorScheme, WarnColorScheme } from './color-pallete.ansi';
import { IColroScheme as IColorScheme } from './scheme';

export class MyLogger implements LoggerService {
    logFile = null;
    filePath = './logs/admin.log';
    constructor() {
        if (!existsSync('./logs')) {
            mkdirSync('./logs')
        }
        ...
        if (this.logFile == null)// to fix this error: EMFILE: too many open file
    ...
    verbose(message: any, context = 'VERBOSE') {
        this.basicLog(message, context, 'VERBOSE', VerboseColorScheme); // whatever that means
    }
    ...
    basicLog(message, context, type, colorScheme = null) {
        ...
        const customMessage = `${messageColor}[Admin] ${process.pid}  - \x1B[39m${this._getDate()}     ${messageColor}${type}\x1B[39m ${contextColor}[${context}]\x1B[39m ${messageColor}${message}\x1B[39m`;
        console.log(customMessage);
    }

    APIlog(message: any, context: any = 'API', reqInfo: any, statusCode: number, error = null) {
    ...
    LogRequest(req){
        this.logFile.write(format('', req) + '\n');
    }

    private _logA(message) {// writing to file; A stands for apple
    ...
    private _formatMessageForLogFile(req: any, statusCode = 500) {
        ...
        return `[HOST] ${hostname} - ${this._getDate()} [STATUS] ${statusCode} - [METHOD] ${method} - [URL] ${originalUrl} - [IP] ${ip} - [USER-AGENT] ${headers['user-agent']}`;
    }
