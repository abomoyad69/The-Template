...
export class AuthService {
  ...
  async validateUser(userCredential: string, pass: string): Promise<any> {
    // userCredential could be username or email
    userCredential = userCredential.trim().toLowerCase();
    const userInfo = await this.usersRepo.findOne({
      where: [
        { email: ILike(userCredential) },
        { username: ILike(userCredential) },
      ],
    });
    ...
    user = await this.usersRepo.findOne({
      where: {
        username: ILike(userCredential),
        userPassword: encryptedPassword.encryptedData,
      },
    });
